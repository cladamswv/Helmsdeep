...
from build_characters import Model
import numpy as np
...
for col,kind in enumerate(['archer','swordsman','spearman','knight']):
 ...
 eye=np.array([2.8,2.6,-6.2] if kind!='knight' else [4.2,3.1,-6.5]);target=np.array([0,1.52,0]);forward=target-eye;forward/=np.linalg.norm(forward);right=np.cross(forward,[0,1,0]);right/=np.linalg.norm(right);up=np.cross(right,forward)
 rel=verts-eye;z=rel@forward;xx=rel@right;yy=rel@up
 scale=1570 if kind!='knight' else 1060;px=(col+.5)*480-(60 if kind=="knight" else 0)+xx/z*scale;py=540-yy/z*scale
 for i in range(0,len(verts),3):
  x=px[i:i+3];y=py[i:i+3];zz=z[i:i+3]
  x0=max(col*480,int(np.floor(x.min())));x1=min((col+1)*480-1,int(np.ceil(x.max())));y0=max(130,int(np.floor(y.min())));y1=min(H-125,int(np.ceil(y.max())))
...
d.text((38,26),'CASTLEHOLD  /  FORGED IN THE VALLEY',font=f,fill='#efcc89');d.text((38,77),'Actual articulated models • CPU material preview; lighting is approximate, not an Android screenshot',font=sm,fill='#c0ccd1')
for i,(name,desc) in enumerate([('ARCHER','Longbow · cowl · leather'),('SWORDSMAN','Arming sword · heater shield'),('SPEARMAN','Long spear · nasal helmet'),('MOUNTED KNIGHT','Visored helm · lance · barding')]):
 d.text((i*480+32,904),name,font=f,fill='#efcc89');d.text((i*480+32,950),desc,font=sm,fill='#c0ccd1')
img.save(Path(__file__).resolve().parents[1]/'docs/medieval-troops-review.png')
